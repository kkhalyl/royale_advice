...
class AdviceEngine:
    ...
    def generate_swap_suggestions(
        ...
        if "Sem feitico" in " ".join(analysis.flagged_issues):
            suggestions.append("Adicione um feitico (Bola de Fogo, Zap ou Veneno) para mais versatilidade e controle.")
        
        if "Sem feitico leve" in " ".join(analysis.flagged_issues):
            suggestions.append("Considere trocar uma carta por um feitico leve (Zap, Tronco ou Bola de Neve) para ciclar mais rapido.")
        
        if "Sem condicao de vitoria" in " ".join(analysis.flagged_issues):
            suggestions.append("Seu deck nao tem uma condicao de vitoria clara (ex: Montador de Porco, P.E.K.K.A, Balao). Adicione uma para dar dano consistente nas torres.")
        
        if "Sem defesa aerea" in " ".join(analysis.flagged_issues):
            suggestions.append("Adicione defesa aérea (Dragão Infernal, Caçador ou Dragão Elétrico) para segurar tropas voadoras.")
        
        if "Elixir muito alto" in " ".join(analysis.flagged_issues):
            suggestions.append("Troque uma carta cara por uma alternativa mais barata para melhorar o ciclo e ter defesas mais consistentes.")
        
        if "Muitas cartas" in " ".join(analysis.flagged_issues):
            suggestions.append("Diversifique a raridade das cartas para ter trocas mais equilibradas de nível e não sofrer tanto contra counters upados.")
